...
ARM = r"""JSON.stringify((()=>{
  const selector=location.hostname==='ytboob.com'?'#ke-grid':'#hs-grid';
  if (!document.querySelector(selector)||document.querySelector('video')) throw Error('Open home before arming');
  window.__grBackWatch?.stop();
  const getBoot=()=>window.__galleryExtensionBoot||window.__kmExtensionBoot;
  const node=document.querySelector(selector), boot=getBoot();
  ...
  const click=e=>{if(e.target.matches?.('.hs-thumb')||(e.target.closest?.('.ke-card')&&!e.target.closest?.('button')))emit('thumbnail-click');};
  const hide=e=>emit('pagehide',{persisted:e.persisted});
  const show=e=>{emit('pageshow',{persisted:e.persisted,sameNode:node===document.querySelector(selector),sameBoot:boot===getBoot()});requestAnimationFrame(()=>requestAnimationFrame(()=>emit('restored-frame')));};
...
async def main():
    ...
    try:
        ...
        matches=[p for p in pages if p.application.bundle=='com.apple.mobilesafari' and urlparse(p.page.web_url).hostname in ('hitomi.la','imhentai.xxx','ytboob.com')]
        ...
        print('FINAL',await asyncio.wait_for(session.runtime_evaluate("JSON.stringify({href:location.href,boot:window.__galleryExtensionBoot||window.__kmExtensionBoot,events:window.__grBackWatch?.events,historyLength:history.length})"),10),flush=True)
    finally:
        if session:
