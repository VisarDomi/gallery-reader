...
DEBUG_USERSCRIPT = ROOT / "gallery-reader-debug.user.js"
MAX_BODY = 128 * 1024
MAX_ITEMS = 500
...
def setup(port: int) -> None:
    ...
    print(f"iPhone debugger:   https://{host}:{port}/gallery-reader-debug.user.js")
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/gallery-reader-debug.user.js":
        ...
        if parsed.path == "/__debug_info":
            ...
            self.send_json({
                "host": host,
                "port": port,
                "debuggerUrl": f"https://{host}:{port}/gallery-reader-debug.user.js",
...
def main() -> None:
    ...
    parser.add_argument("--port", type=int, default=int(os.environ.get("IOS_DEBUG_PORT", "38888")))
